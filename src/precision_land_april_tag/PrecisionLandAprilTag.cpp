#include "PrecisionLandAprilTag.hpp"
...
static const std::string kModeName = "PrecisionLandAprilTag";
static const bool kEnableDebugOutput = true;

using namespace px4_ros2::literals;

PrecisionLandAprilTag::PrecisionLandAprilTag(rclcpp::Node& node)
	...
	, _node(node)
{
	...
	_vehicle_land_detected_sub = _node.create_subscription<px4_msgs::msg::VehicleLandDetected>("/fmu/out/vehicle_land_detected",
				     rclcpp::QoS(1).best_effort(), std::bind(&PrecisionLandAprilTag::vehicleLandDetectedCallback, this, std::placeholders::_1));

	loadParameters();
}

void PrecisionLandAprilTag::loadParameters()
{
...
void PrecisionLandAprilTag::vehicleLandDetectedCallback(const px4_msgs::msg::VehicleLandDetected::SharedPtr msg)
{
...
void PrecisionLandAprilTag::onActivate()
{
...
void PrecisionLandAprilTag::onDeactivate()
{
	// No-op
}

void PrecisionLandAprilTag::updateSetpoint(float dt_s)
{
...
Eigen::Vector2f PrecisionLandAprilTag::calculateVelocitySetpointXY()
{
...
bool PrecisionLandAprilTag::checkTargetTimeout()
{
	if (!_tag.valid()) {
		return true;
	}
...
void PrecisionLandAprilTag::generateSearchWaypoints()
{
...
bool PrecisionLandAprilTag::positionReached(const Eigen::Vector3f& target) const
{
...
std::string PrecisionLandAprilTag::stateName(State state)
{
	switch (state) {
	case State::Idle:
		return "Idle";
...
void PrecisionLandAprilTag::switchToState(State state)
{
...
int main(int argc, char* argv[])
{
	...
	rclcpp::spin(std::make_shared<px4_ros2::NodeWithMode<PrecisionLandAprilTag>>(kModeName, kEnableDebugOutput));
	rclcpp::shutdown();
	return 0;
}
